...
if(isset($_POST["inputJsonPDF"])){
	...
	$data = json_decode($_POST["inputJsonPDF"], true);
	
	$zip = new ZipArchive();
	$dir = __DIR__ . '/tmp/';
	$nameZip = "ENTREVISTAS-GESTIONADAS-PDF-MASIVO-".datetimeNowName() . ".zip";
	$filename = $dir . $nameZip;
	...
	foreach ($data['data'] as $key => $value) {
		...
		$documento = $id[1];
		$codprograma = $programa[0];
		$nameprograma = $programa[1];
		$codperiodo = $_POST["codperiodo"];


		$nombrePDFTmp = $dir . 'ENTREVISTA-'.$codperiodo.'-'.$documento.'-'.replaceSpace($value[3]).'-PROGRAMA-'.$codprograma.'-'.replaceSpace($nameprograma).'.pdf';
		if (!file_exists($nombrePDFTmp)) {
			...
			$nombrePDF = 'ENTREVISTA-'.$codperiodo.'-'.$dataEntrevista['codestudiante'].'-'.replaceSpace($dataEntrevista['nombrecompleto']).'-PROGRAMA-'.$dataEntrevista['codprograma'].'-'.replaceSpace($dataEntrevista['nombreprograma']);
			...
			$dirprograma = '/' . $dataEntrevista['codprograma'] . '-' . replaceSpace($dataEntrevista['nombreprograma']);
			$dirstudent =  $dirprograma . '/' . replaceSpace($dataEntrevista['nombrecompleto']) . '-' . $documento;
			$routeInZip = replaceSpace($value[0]) . $dirstudent;
			$namePDF = $dir . $nombrePDF.'.pdf';
			
			// create new PDF documentn     
			...
			$nombrePDF = 'ENTREVISTA-'.$codperiodo.'-'.$documento.'-'.replaceSpace($value[3]).'-PROGRAMA-'.$codprograma.'-'.replaceSpace($nameprograma);
			
			$dirprograma = '/' . $codprograma . '-' . replaceSpace($nameprograma);
			$dirstudent =  $dirprograma . '/' . replaceSpace($value[3]) . '-' . $documento;
			$routeInZip = replaceSpace($value[0]) . $dirstudent;
			$namePDF = $nombrePDFTmp;
		}
		
		$zip->addFile($namePDF, $routeInZip . '/' . $nombrePDF.'.pdf');
